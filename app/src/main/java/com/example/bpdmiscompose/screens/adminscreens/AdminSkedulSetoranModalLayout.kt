...
@Composable
fun AdminSkedulSetoranModalLayout (modifier: Modifier = Modifier, navController: NavHostController, skedulSetoranViewModel: SkedulSetoranViewModel
) {
    ...
    when(skedulSetoranUiState.setoranList){
        ...
        is Resources.Success -> {
            ...
            if(!hasRedirected.value){
                when(pageSelected.value){
                    0 -> {
                        navController.navigate(BPDMISScreen.AdminSkedulBerdasarkanTahun.name)
                        hasRedirected.value = true
                    }
                    1 -> {
                        navController.navigate(BPDMISScreen.AdminSkedulBerdasarkanPemda.name)
                        hasRedirected.value = true
                    }
                    2 -> {
                        navController.navigate(BPDMISScreen.AdminSkedulCRUD.name)
                        hasRedirected.value = true
                    }
                }
            }
        ...
        is Resources.Error -> {
            circularState.value = false
            Log.e(TAG, skedulSetoranUiState.setoranList.throwable?.message.toString())
    ...
    LazyColumn (
                ...
                Column(
                    ...
                    Button(onClick = {
                        try{
                            ...
                            Log.i(TAG, "the selected pemegang saham is ${pemegangSahamSelectedItem.value}")
                            hasRedirected.value = false
        ...
        item {
            ...
            OutlinedButton(onClick = {
                ...
                Text(
                    text = stringResource(id = R.string.tampilkan_tambahkan_ubah_skedul),
                    textAlign = TextAlign.Center
                )
            }
        }
