...
@Composable
fun StaffSkedulSetoranModalLayout (modifier: Modifier=Modifier, navController: NavHostController){
    LazyColumn (
        ...
        item {
            OutlinedButton(onClick = {navController.navigate(BPDMISScreen.StaffSemuaSetoran.name)} , colors = ButtonDefaults.buttonColors(backgroundColor = Color.Transparent, contentColor = MaterialTheme.colors.primary), border = BorderStroke(width = 1.dp, color = MaterialTheme.colors.primary), modifier = Modifier.fillMaxWidth()) {
                Text(text = stringResource(id = R.string.tampilkan_semua_data))
            }
        }
    }
}
