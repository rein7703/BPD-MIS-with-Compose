...
@Composable
fun AdminIndikatorUpdateRBB(rbbViewModel: RBBViewModel = hiltViewModel()){
    val rbbUiState = rbbViewModel.rbbUiState
    val tahunChosen = remember{ mutableStateOf(rbbUiState.rbbChosen.data?.tahun.toString()) }
    val jenisKinerjaChosen = remember { mutableStateOf(rbbUiState.rbbChosen.data?.jenisKinerja.toString()) }
    val kantorChosen = remember { mutableStateOf(rbbUiState.rbbChosen.data?.kantor.toString()) }
    val nominalChosen = remember { mutableStateOf(rbbUiState.rbbChosen.data?.nominal.toString()) }
    val rbbId = remember { mutableStateOf(rbbUiState.rbbChosen.data?.rbbid ?: "") }
    ...
    LazyColumn(modifier = Modifier.padding(20.dp)){
        ...
        item{
            Text(
                text = stringResource(id = R.string.periode),
